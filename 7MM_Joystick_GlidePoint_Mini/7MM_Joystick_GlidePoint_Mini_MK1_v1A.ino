// 7MM_Joystick_GlidePoint_Mini_v1A
// 2023-05-19-A
const long buildDate = 2023051901;
...
// 2023-04-16 : For the MKIII, rotated GP so that connector is opposite of USB ("down")
// 2023-05-19 : For the Mini, removed unnecessary code
// 2023-05-19 : Updated min/max x/y ranges based on actual data
// 2023-05-19 : Cleaned up limiting and mapping code
...
// CIRQUE: Coordinate scaling values -- these are static, based on datasheet specs
...
const int minHorz=400;
const int maxHorz=1100;
const int minVert=300;
const int maxVert=1300;
...
void loop() {
  ...
  {    
    ...
    ScaleData(&touchData, PINNACLE_X_RANGE, PINNACLE_Y_RANGE);  // Scale coordinates to X, Y resolution
    ...
    //MKIII/Mini has the alignment changed. Now 90 degree flipped/rotated -- when viewed from face
    //rawHorz = (int) touchData.xValue; // MKII and prior -- FFC on -X "left"
    //rawVert = (int) touchData.yValue; // MKII and prior -- FFC on -X "left
    rawHorz = (int) touchData.yValue; // MKIII and higher -- FFC on -Y "down", X/Y rotated
    rawVert = (int) touchData.xValue; // MKIII and higher -- FFC on -Y "down", X/Y rotated
    ...
    mapVert = map(lmtVert, minVert, maxVert, joyMax, joyMin); // Inverted Vertical
    ...
    if ( usb_hid.ready() && touchData.touchDown ) {
      joystick.x = mapHorz;
      joystick.y = mapVert;
