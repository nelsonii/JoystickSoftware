// 7MM_Joystick_JoyConWithAuxButton_QTPy_SAMD21_v3A
// 2022-11-08-A
const long buildDate = 2022110801;
...
// 2022-11-08 : Added ability to have an AUX button
// 2022-11-09 : When plugged into Android devices, the button mapping is:
//              GAMEPAD_BUTTON_0 = "A" = "SOUTH" = "CLICK"
//              GAMEPAD_BUTTON_1 = "B" = "EAST" = "BACK"
...
// Joystick pins
#define AUX_PIN     A0 // Button for aux use (ex. "back")
...
int SELAux;
int prevSELAux = HIGH;
long lastDebounceTimeAux = 0;
const int debounceDelayAux = 50;  //millis
...
void loop() {
  ...
  // Since input is "pulled up", the pin goes low on press.
  ...
  // Check for AUX button push - and debounce
  // Since input is "pulled up", the pin goes low on press.
  int readingAux = digitalRead(AUX_PIN);
  if (readingAux != prevSELAux) {
     lastDebounceTimeAux = millis();// reset timer
  }
  if (millis() - lastDebounceTimeAux > debounceDelayAux) {
    if (readingAux != SELAux) {
      SELAux = readingAux;
      if (SELAux == LOW) {
        btnAux = true;
      }
  ...
  // Map values to a range the upstream device likes
  ...
  // Ready to update the HID
  if ( usb_hid.ready() ) {
      //Set joystick object's X and Y
      joystick.x = mapHorz;
      joystick.y = mapVert;
      //Set joystick objects button selections
      //Only Stick Pressed
      if (btnStick && !btnAux) { joystick.buttons = GAMEPAD_BUTTON_0; }
        //Only Aux pressed
        else if (!btnStick && btnAux) { joystick.buttons = GAMEPAD_BUTTON_1; }
        //Both Stick and Aux pressed
        else if (btnStick && btnAux) {joystick.buttons = GAMEPAD_BUTTON_0 + GAMEPAD_BUTTON_1; }
        //Nothing pressed
        else joystick.buttons = 0;
      //Send the updated stick info to upstream HID
...
void serialDebug() {
    ...
    Serial.print(" ");
    if (!graph) {Serial.print("Aux: ");}
    //always graph/print aux
    Serial.print(digitalRead(AUX_PIN));
    Serial.print(" ");
